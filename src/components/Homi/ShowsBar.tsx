...
import { elementShow } from "../../types/TypeMovieElements";
          ...
          >
            {Show.map((element) => (
              <SwiperSlide key={element.id} className="">
                <div
                  id={`${tipo}_${element.id}`}
                  className="h-72 bigSize:h-80 midize:h-80 flex items-center justify-center top-0 bottom-0"
                >
                  <Link
                    to={`/${tipo}/${element.id}`}
                  ...
                  >
                    <img
                      src={
                        element.poster_path
                          ? linksUrl.image + element.poster_path
                          : NotFound
                      }
                      alt={
                        element.poster_path
                          ? element.title
                          : `Not Found Image of: ${element.title}`
                      }
